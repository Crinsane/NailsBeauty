var SUMERIANLAB_googlemap_init_obj = {};
var SUMERIANLAB_GEOCODE_ERROR = "Error";
// Google map Styles
var SUMERIANLAB_googlemap_styles = {
    'default': [],
...
function googlemap_init(dom_obj, coords) {
    ...
    try {
        ...
        var id = dom_obj.id;
        SUMERIANLAB_googlemap_init_obj[id] = {};
        SUMERIANLAB_googlemap_init_obj[id].dom = dom_obj;
        SUMERIANLAB_googlemap_init_obj[id].point = coords.point;
        SUMERIANLAB_googlemap_init_obj[id].description = coords.description;
        SUMERIANLAB_googlemap_init_obj[id].title = coords.title;
        SUMERIANLAB_googlemap_init_obj[id].opt = {
            zoom: coords.zoom,
            center: latlng,
            scrollwheel: false,
            scaleControl: false,
            ...
            styles: SUMERIANLAB_googlemap_styles[coords.style ? coords.style : 'default'],
        ...
        custom_map.geocode(coords.latlng!=='' ? {'latLng': latlng} : {"address": coords.address}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                SUMERIANLAB_googlemap_init_obj[id].address = results[0].geometry.location;
                googlemap_create(id);
            } else {
                alert(SUMERIANLAB_GEOCODE_ERROR + ' ' + status);
            }
        });

        jQuery(window).resize(function() {
            if (SUMERIANLAB_googlemap_init_obj[id].map) {
                SUMERIANLAB_googlemap_init_obj[id].map.setCenter(SUMERIANLAB_googlemap_init_obj[id].address);
            }
        });
...
function googlemap_create(id) {
    "use strict";
    if (!SUMERIANLAB_googlemap_init_obj[id].address) { 
        return false;
    }
    SUMERIANLAB_googlemap_init_obj[id].map = new google.maps.Map(SUMERIANLAB_googlemap_init_obj[id].dom, SUMERIANLAB_googlemap_init_obj[id].opt);
    SUMERIANLAB_googlemap_init_obj[id].map.setCenter(SUMERIANLAB_googlemap_init_obj[id].address);
    var markerInit = {
        map: SUMERIANLAB_googlemap_init_obj[id].map,
        position: SUMERIANLAB_googlemap_init_obj[id].address   //SUMERIANLAB_googlemap_init_obj[id].map.getCenter()
    };
    if (SUMERIANLAB_googlemap_init_obj[id].point) { 
        markerInit.icon = SUMERIANLAB_googlemap_init_obj[id].point;
    }
    if (SUMERIANLAB_googlemap_init_obj[id].title) { 
        markerInit.title = SUMERIANLAB_googlemap_init_obj[id].title;
    }
    var marker = new google.maps.Marker(markerInit);
    var infowindow = new google.maps.InfoWindow({
        content: SUMERIANLAB_googlemap_init_obj[id].description
    });
    google.maps.event.addListener(marker, "click", function() {
        infowindow.open(SUMERIANLAB_googlemap_init_obj[id].map, marker);
    });
}


function googlemap_refresh(){
    for(id in SUMERIANLAB_googlemap_init_obj){
        googlemap_create(id);
    }
}
